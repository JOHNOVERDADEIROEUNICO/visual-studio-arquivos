...
class Program
{
    static void Main()
    {
        ...
        byte[] Hash = GerarHashSha256(Senha);//OBS IMPORTANTE: QUANDO PASSAMOS O HASH NO FORMATO DE STRING, A VARIÁVEL DO BANCO DEVE SER DO TIPO CHAR DE TAMANHO 64, SE PASSARMOS O HASH NO SEU FORMATO BINÁRIO A VARIÁVEL DO BANCO DEVE SER DO TIPO BINARY DE TAMANHO 32. ALÉM DISSO A MANEIRA DE PASSAR O CÓDIGO NA PARTE DO CMD.PARAMETERS MUDA.

        using (MySqlConnection mysql = new MySqlConnection(conexao))
        {
            mysql.Open();

            string comandoInsert = "INSERT INTO usersbinary (nome, senha_hash2) VALUES (@nome, @senha_hash2)";
            using (MySqlCommand cmd = new MySqlCommand(comandoInsert, mysql))
            {
                cmd.Parameters.AddWithValue("@nome", Nome);
                cmd.Parameters.AddWithValue("@senha_hash2", Hash);
    ...
    static byte[] GerarHashSha256(string senha)
    {
