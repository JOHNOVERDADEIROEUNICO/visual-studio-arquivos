...
public class Usuario
{
    ...
    public static List<Usuario> Usuarios { get; private set; } = Guardar.Carregar();
    ...
    public void GerarHash()
    {
        using (SHA256 sha256 = SHA256.Create())
        {
            ...
            HashSenha = BitConverter.ToString(hashSenhaByte).Replace("-", "").ToLower();
            Senha = string.Empty;
        }
    }
        ...
        else
        {
            ...
            while (true)
            {
                ...
                using (SHA256 sha256 = SHA256.Create())
                {
                    ...
                    password = BitConverter.ToString(hashSenhaByte).Replace("-", "").ToLower();
                }


                Usuario? senha = Usuarios.Find(u => u.HashSenha == password);

                if (senha != null)
                {
                    Usuarios.Remove(senha);
                    Guardar.Salvar(Usuarios);
    ...
    public void MostrarTodosUsuarios()
    {
        ...
        {
            Usuarios.ForEach(Usuarios => Console.WriteLine(Usuarios));
        }
    }
    ...
    public void FazerLogin()
    {
        ...
        else
        {
            while (true)
            {
                ...
                Usuario? passwordComparisonTrue = Usuarios.Find(n => n.HashSenha == passwordComparison && n.Nome == name);

                if (passwordComparisonTrue != null)
                {
                    Console.WriteLine("Login realizado com sucesso!");
                    passwordComparisonTrue = null;
    ...
    public static Usuario DeDTO(UsuarioDTO usuario)
    {
        return new Usuario
        {
            Nome = usuario.NomeDTO,
            HashSenha = usuario.HashDTO
        };
    }
}
...
public class Login
{
    public void NewLogin()
    {
        ...
        Usuario.Usuarios.Add(novo);
        Guardar.Salvar(Usuario.Usuarios);
    }
}

public static class Guardar
{
    private static string caminho = @"C:\Users\hugol_000\Desktop\vs code\projetos\projetos e arquivos\c# projetos\códigos\desafios\desafio do login\versao com uso de JSON\usuario.json";

    public static void Salvar(List<Usuario> usuarios)
    {
        var listaDTO = usuarios.Select(u => u.ParaDTO()).ToList();
    ...
    public static List<Usuario> Carregar()
    {
        ...
        var listaDTO = JsonSerializer.Deserialize<List<UsuarioDTO>>(json);

        return listaDTO?.Select(dto => Usuario.DeDTO(dto)).ToList() ?? new List<Usuario>();
    }
}

public class UsuarioDTO
{
    public string NomeDTO { get; set; } = string.Empty;
    public string HashDTO { get; set; } = string.Empty;
}